...
# get my IP for ssh rule
ALLOWED_SSH_IP=$(curl -s http://checkip.amazonaws.com)
...
aws ec2 authorize-security-group-ingress --group-id $SECURITY_GROUP_ID --protocol tcp --port 22 --cidr "${ALLOWED_SSH_IP}/32" # For SSH Access
